/*
|--------------------------------------------------------------------------
| Recursos de Mecánica Newtoniana
|--------------------------------------------------------------------------
| Cada objeto representa un subtema del repositorio.
| btns_repositorios.js únicamente leerá esta información.
|--------------------------------------------------------------------------
*/
/*Esta constante se utiliza en btns_repositorios en la funcion pruebaSubtemas*/
const recursosSubtemas = [

    {
    // CODEX: modificado para vincular cada subtema con su banco de evaluacion
        nombre: "Movimiento Rectilíneo Uniforme",
        evaluacion: "cinematica_mru",
        pdf: "Capitulo_7_Fluidos.pdf",
        video: "https://www.youtube.com/watch?v=75xi6aasdw4&list=PLeySRPnY35dF7yGgUKWV2L-03TK1TlBNY"
    },

    {
        nombre: "Movimiento Rectilíneo Uniformemente Variado",
        evaluacion: "cinematica_mruv",
        pdf: "Capitulo_7_Fluidos.pdf",
        video: "https://www.youtube.com/watch?v=_HVgknQ1CXQ&list=PLeySRPnY35dGlWnfMQ-xC_qtn-xO5RWcF"
    },

    {
        nombre: "Movimiento de Proyectiles",
        evaluacion: "cinematica_movimiento_parabolico",
        pdf: "Capitulo_7_Fluidos.pdf",
        video: "https://www.youtube.com/watch?v=vFMHr1Jg8IA"
    }

];
